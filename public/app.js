...
function formatDate(dateStr) {
  if (!dateStr) return "Data nepatikslinta";
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return "Data nepatikslinta";
  ...
  try {
    ...
    data.forEach(g => {
      ...
      div.innerHTML = `
        ...
        <div style="opacity:.7;font-size:14px;">📅 ${formatDate(g.commence_time)}</div>
        ...
        ${
          g.total
            ? `<div class="market">
                🏷 O/U: <b>${g.total.pick}</b> ${g.total.line}
